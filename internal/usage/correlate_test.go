...
type fakeReader struct{ calls map[string][]provider.Call }
...
func TestReconcileAmbiguousSharedCWD(t *testing.T) {
	...
	rd := fakeReader{calls: map[string][]provider.Call{"/repo": {{InputTokens: 100}}}}
	if rep := Reconcile(events, []provider.Reader{rd}); len(rep) != 0 {
