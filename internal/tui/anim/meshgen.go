...
// meshgen bakes a 3D head glTF into a compact embedded point cloud
// (mesh_data.go). It is stdlib-only and excluded from normal builds.
//
// Run from this directory:
//
//	go run meshgen.go
//
// or via `go generate ./internal/tui/anim/` (see the directive in mesh.go).
//
// Source asset (NOT committed — see .gitignore): testdata/scene.gltf +
// testdata/scene.bin. Attribution for the source model is in the repo NOTICE.
package main
...
func main() {
	...
	// absent. Skip cleanly in that case — mesh_data.go is committed and is
	// the source of truth — so `go generate ./...` stays green for everyone.
	if _, err := os.Stat("testdata/scene.gltf"); os.IsNotExist(err) {
		fmt.Fprintln(os.Stderr, "meshgen: testdata/scene.gltf absent; keeping committed mesh_data.go")
		return
	}
	...
	b.WriteString("//\n")
	b.WriteString("// Point cloud baked from the source 3D model; attribution is in the repo NOTICE.\n\n")
	b.WriteString("package anim\n\n")
	fmt.Fprintf(&b, "// meshPoints is the decimated head surface (%d points).\n", len(out))
	b.WriteString("var meshPoints = []meshPoint{\n")
	for _, p := range out {
		fmt.Fprintf(&b, "\t{%s, %s, %s, %s, %s, %s, %s},\n",
			ff(p.x), ff(p.y), ff(p.z), ff(p.nx), ff(p.ny), ff(p.nz), regionConst(p.region))
	}
	b.WriteString("}\n")
	must(os.WriteFile("mesh_data.go", []byte(b.String()), 0o644))
	fmt.Fprintln(os.Stderr, "wrote mesh_data.go")
}
